<template>
  <div class="container">
    <div class="time">
      当前时间:{{formatTime}}
      <a-button class="fr">
        <router-link to="/fullScreen">全屏</router-link>
      </a-button>
    </div>
...
import { Index } from '@/service/monitoring'
...
export default {
  ...
  methods: {
    ...
    initWebSocket () { // 初始化weosocket
      const wsUrl = 'ws://192.168.0.127:9090?key=PC'
    ...
    websocketonmessage (e) { // 数据接收
      ...
      console.log(data)
      if (data.ApiMethod === 'GetPitData') {
        this.data = data.Data.JobRegistrations
        this.oxygenGroup = data.Data.OxygenGroup
        this.temperatureGroup = data.Data.TemperatureGroup
        this.humidityGroup = data.Data.HumidityGroup
      }
    },
    websocketclose (e) { // 关闭
      console.log('断开连接', e)
